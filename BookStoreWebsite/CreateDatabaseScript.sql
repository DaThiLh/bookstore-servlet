...
CREATE TABLE `promotion` (
    ...
    `promotionDateStart` datetime NOT NULL.
    `promotionDateEnd` datetime NOT NULL,
    `promotionName` nvarchar(256) NOT NULL,
    `discount` float NOT NULL,
    `maximumOrders` int(11) NOT NULL,
    `canApply` boolean NOT NULL,
    PRIMARY KEY (`promotionId)`,
    UNIQUE KEY `promotionId_UNIQUE` (`order_id`),
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=utf8;

CREATE TABLE PromotionOrder (
   `promotionOrderId` int(11) NOT NULL AUTO_INCREMENT,
   `promotionId` int(11) NOT NULL,
   `order_id` int(11) NOT NULL,
   `customerId` int(11),
   `postTotal` float NOT NULL,
   PRIMARY KEY (`promotionOrderId`),
   UNIQUE KEY `promotionOrderId_UNIQUE` (order_id),
   KEY `customer_fk_2_idx` (`customer_id`),
   KEY `order_fk_idx` (`order_id`),
   KEY `promotionId_fk_idx` (`promotionId`),
   CONSTRAINT `customer_fk_2` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`customer_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
   CONSTRAINT `order_fk` FOREIGN KEY (`order_id`) REFERENCES `book_order` (`order_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
   CONSTRAINT `promotionId_fk` FOREIGN KEY (`promotionId`) REFERENCES `promotion` (`promotionId`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=utf8;
